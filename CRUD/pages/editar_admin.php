...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $ADM_ATIVO = $_POST['ADM_ATIVO'] ?? '1';

    try {
        if ($ADM_SENHA !== '') {
            $senhaHash = password_hash($ADM_SENHA, PASSWORD_DEFAULT);
            $sql = "UPDATE ADMINISTRADOR SET ADM_NOME = :ADM_NOME, ADM_EMAIL = :ADM_EMAIL, ADM_SENHA = :ADM_SENHA, ADM_ATIVO = :ADM_ATIVO WHERE ADM_ID = :ADM_ID";
            $stmt = $pdo->prepare($sql);
            $stmt->bindParam(':ADM_ID', $ADM_ID, PDO::PARAM_INT);
            $stmt->bindParam(':ADM_NOME', $ADM_NOME, PDO::PARAM_STR);
            $stmt->bindParam(':ADM_EMAIL', $ADM_EMAIL, PDO::PARAM_STR);
            $stmt->bindParam(':ADM_SENHA', $senhaHash, PDO::PARAM_STR);
            $stmt->bindParam(':ADM_ATIVO', $ADM_ATIVO, PDO::PARAM_STR);
        } else {
            $sql = "UPDATE ADMINISTRADOR SET ADM_NOME = :ADM_NOME, ADM_EMAIL = :ADM_EMAIL, ADM_ATIVO = :ADM_ATIVO WHERE ADM_ID = :ADM_ID";
            $stmt = $pdo->prepare($sql);
            $stmt->bindParam(':ADM_ID', $ADM_ID, PDO::PARAM_INT);
            $stmt->bindParam(':ADM_NOME', $ADM_NOME, PDO::PARAM_STR);
            $stmt->bindParam(':ADM_EMAIL', $ADM_EMAIL, PDO::PARAM_STR);
